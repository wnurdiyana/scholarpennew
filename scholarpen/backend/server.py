"""ScholarPen — Q1-grade AI manuscript writer (portable, free-deployable).

Stack: FastAPI + MongoDB + direct Anthropic SDK. No proprietary deps.
...
- Per-section chat: /manuscripts/{id}/sections/{section_key}/chat
- Data Lab: /manuscripts/{id}/datalab/upload, /suggest, /plot, /datasets
- Figure critique: /manuscripts/{id}/figure/critique
- References (Crossref): /references/search
- Export: /manuscripts/{id}/export?format=md|docx|pdf
"""
...
from fastapi import (
    APIRouter, Cookie, Depends, FastAPI, File, Form, Header, HTTPException,
...
from llm import chat_text, generate_text, vision_text
...
app = FastAPI(title="ScholarPen API", version="1.0.0")
api = APIRouter(prefix="/api")

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(name)s :: %(message)s")
logger = logging.getLogger("scholarpen")


# ---------- Section catalog ----------
SECTION_KEYS = [
    "title", "abstract", "keywords", "introduction", "literature_review",
    "methodology", "experimental_setup", "math_modeling", "ml_framework",
    "results", "discussion", "comparison", "practical_implications",
    "limitations", "conclusion", "future_work", "nomenclature", "references",
]
...
def _make_jwt(user_id: str) -> str:
    payload = {"sub": user_id, "iat": _now(), "exp": _now() + timedelta(days=JWT_EXPIRY_DAYS)}
...
def _public_user(u: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "user_id": u["user_id"], "email": u.get("email", ""), "name": u.get("name", ""),
...
async def register(payload: RegisterIn):
    ...
    doc = {
        "user_id": user_id, "email": payload.email.lower(), "name": payload.name.strip(),
        "picture": None, "password_hash": pw_hash, "auth_provider": "password",
        "created_at": _iso(_now()),
    }
    await db.users.insert_one(doc)
    return {"token": _make_jwt(user_id), "user": _public_user(doc)}
...
async def login(payload: LoginIn):
    ...
    if not ok:
        raise HTTPException(status_code=401, detail="Invalid email or password")
    return {"token": _make_jwt(user["user_id"]), "user": _public_user(user)}
...
async def create_manuscript(payload: ManuscriptCreateIn, user: Dict[str, Any] = Depends(current_user)):
    ...
    doc = {
        ...
        "sections": _empty_sections(), "references": [],
...
- Free from generic filler, exaggerated claims, vague wording.
...
- For multi-item ordered lists, put items on consecutive lines with NO blank lines between them.
"""
...
def _section_prompt(section_key: str, manuscript: Dict[str, Any], extra: str) -> str:
    ...
    context_parts = []
    for ck in ["title", "abstract", "introduction", "methodology", "results"]:
        if ck == section_key:
            continue
...
5. ...
""",
        "abstract": """Produce a 200–250 word structured Abstract covering: background/problem, gap, objective, methodology, key quantitative findings, scientific significance, practical contribution. No citations. No undefined abbreviations. Single flowing paragraph.""",
        "keywords": """Provide 5–8 high-impact keywords aligned with WoS/Scopus indexing. Comma-separated.""",
        "introduction": """Write a critical, analytical Introduction (~1100–1500 words) that ALSO contains a Research Gap subsection and a Novelty Statement subsection. Structure (use exact sub-headings):
### Background and motivation
### State of the art
### Research gap
### Novelty and contribution
### Objectives""",
        "literature_review": """Write a critical Literature Review (~900–1300 words). Synthesize, do not list. End with a "### Comparative Synthesis" sub-heading and a Markdown table: Reference | Approach | Dataset / Setting | Key Finding | Limitation (≥8 rows). Use inline [Author, Year] citations.""",
        "methodology": """Write a reproducible Methodology (~900–1200 words). Use ### sub-headings. Include at least one LaTeX-style equation.""",
        "experimental_setup": """Describe the Experimental Setup (~500–800 words). Use ### sub-headings; add a Markdown table of key parameters and ranges.""",
        "math_modeling": """Mathematical / Statistical Modeling (~600–900 words). LaTeX-numbered equations; state assumptions; solver / estimation procedure; convergence criteria.""",
        "ml_framework": """Machine Learning Framework (~600–900 words). Cover dataset prep, train/val/test split, architecture, hyperparameters (table), optimizer, activations, loss, regularization, metrics. If ML is not applicable, say so in one short paragraph and explain why.""",
        "results": """Write Results (~900–1200 words). Quantitative trends; reference Fig. 1, Table 1 with italicized caption suggestions; include at least one Markdown table.""",
        "discussion": """Write a deep, mechanistic Discussion (~900–1300 words). Benchmark against literature with [Author, Year]. Engineering, sustainability, policy implications. Do not restate results.""",
        "comparison": """Comparison with Existing Literature (~400–600 words) plus a Markdown table: Study | Method | Reported Metric | This Study | Δ Improvement.""",
        "practical_implications": """Practical Implications (~300–500 words). Industry adoption, regulatory relevance, deployment, scalability, economics.""",
        "limitations": """Limitations (~250–400 words). Be candid about data scope, model assumptions, generalizability.""",
        "conclusion": """Conclusion (400–700 words). Summarize quantitative findings, emphasize contribution and significance, brief limitations, future-work hooks. No new citations.""",
        "future_work": """Future Recommendations (~250–400 words) as numbered, specific, feasible research directions.""",
        "nomenclature": """Produce a Nomenclature section as a Markdown table: Symbol | Description | Unit. Group by Roman / Greek / Subscripts / Superscripts / Abbreviations using ### sub-headings.""",
        "references": f"""Compile a References section in **{citation_style}** style. Prioritize 2021–2026 Q1/Q2 WoS- or Scopus-indexed sources; ~10–20% foundational. Provide 25–45 entries. Mark unverifiable DOIs with " — verify DOI". Do NOT fabricate DOIs.""",
    }
...
async def generate_section(
    mid: str, section_key: str, payload: SectionGenerateIn,
    ...
    await db.manuscripts.update_one(
        {"manuscript_id": mid},
        {"$set": {f"sections.{section_key}.status": "generating", "updated_at": _iso(_now())}},
    )

    prompt = _section_prompt(section_key, doc, payload.extra_instructions or "")
    try:
        response_text = await generate_text(SYSTEM_PROMPT, prompt)
        if response_text.startswith("```"):
            response_text = re.sub(r"^```[a-zA-Z]*\n?", "", response_text)
            response_text = re.sub(r"\n?```$", "", response_text)
    except Exception as exc:
        logger.exception("LLM generation failed")
        await db.manuscripts.update_one({"manuscript_id": mid}, {"$set": {f"sections.{section_key}.status": "error"}})
        raise HTTPException(status_code=502, detail=f"Generation failed: {exc}")

    await db.manuscripts.update_one(
        {"manuscript_id": mid},
        {"$set": {
            f"sections.{section_key}.content": response_text.strip(),
    ...
    if section_key == "title":
        ...
        if m:
            ...
            if new_title:
                await db.manuscripts.update_one({"manuscript_id": mid}, {"$set": {"title": new_title[:200]}})
...
# ---------- Section chat ----------
SECTION_CHAT_SYSTEM = """You are an academic writing collaborator embedded in a Q1-grade manuscript editor. Your job is to help the author refine ONE specific section.

You receive the section context, current Markdown content, the conversation, and a new user message. Decide:
(a) discussion/question → reply conversationally; do not change content.
(b) content modification → produce the FULL revised section in clean Markdown and a short explanatory reply.

ALWAYS respond with a single JSON object on a single line, no code fences:
{"reply": "<1-4 short sentences>", "updated_content": "<full revised Markdown or null>"}

Rules:
- For (a) set updated_content to null. For (b) put the FULL replacement Markdown there.
- Use ### sub-headings for subtopics (never bold paragraphs).
- Never fabricate DOIs or unrealistic numbers.
...
async def section_chat(
    mid: str, section_key: str, payload: SectionChatIn,
    user: Dict[str, Any] = Depends(current_user),
):
    if section_key not in SECTION_KEYS:
        raise HTTPException(status_code=400, detail="Unknown section key")
    ...
    history_block = "\n".join(f"[{m.get('role','user').upper()}] {m.get('content','')}" for m in recent) or "_(no prior turns)_"

    prompt = f"""# Section being refined
**{SECTION_LABELS.get(section_key)}** — for {inputs.get('journal_target') or 'a Q1 WoS-indexed journal'} in {inputs.get('field') or '(unspecified field)'}. Citation style: {inputs.get('citation_style') or 'APA'}.
...
"""
    try:
        raw = await chat_text(SECTION_CHAT_SYSTEM, prompt)
    except Exception as exc:
    ...
    fresh = await db.manuscripts.find_one({"manuscript_id": mid}, {"_id": 0})
    return {"section": fresh["sections"][section_key], "reply": reply_text, "content_updated": bool(updated)}


# ---------- Data Lab ----------
@api.post("/manuscripts/{mid}/datalab/upload")
async def datalab_upload(
    mid: str, file: UploadFile = File(...),
    user: Dict[str, Any] = Depends(current_user),
):
    doc = await db.manuscripts.find_one({"manuscript_id": mid, "user_id": user["user_id"]}, {"_id": 0})
    if not doc:
    ...
    await db.manuscripts.update_one(
        {"manuscript_id": mid},
        {"$push": {"datasets": {
            "dataset_id": dataset_id, "filename": filename, "uploaded_at": _iso(_now()),
            "rows": summary["shape"]["rows"], "cols": summary["shape"]["cols"],
...
async def datalab_suggest(mid: str, payload: DataSuggestIn, user: Dict[str, Any] = Depends(current_user)):
    doc = await db.manuscripts.find_one({"manuscript_id": mid, "user_id": user["user_id"]}, {"_id": 0})
    if not doc:
        raise HTTPException(status_code=404, detail="Manuscript not found")
...
Return the strict JSON object now."""
    try:
        raw = await chat_text(DATA_SUGGEST_SYSTEM, prompt)
    except Exception as exc:
...
async def datalab_plot(mid: str, payload: DataPlotIn, user: Dict[str, Any] = Depends(current_user)):
    doc = await db.manuscripts.find_one({"manuscript_id": mid, "user_id": user["user_id"]}, {"_id": 0})
    if not doc:
        raise HTTPException(status_code=404, detail="Manuscript not found")
    ...
    df = load_dataframe(path)
    try:
        return render_plot(df, payload.plot_type, x=payload.x, y=payload.y, hue=payload.hue, title=payload.title)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
async def figure_critique(
    ...
    media_type = (image.content_type or "image/png").lower()
    ...
    if dataset_id:
        ...
        if path:
            try:
                df = load_dataframe(path)
                dataset_context = summary_to_llm_text(summarize_dataframe(df), path.name)
            except Exception:
                df = None
...
Critique the uploaded figure and return the strict JSON object now."""
    try:
        raw = await vision_text(FIGURE_CRITIQUE_SYSTEM, prompt, image_b64, media_type=media_type)
    except Exception as exc:
        ...
        if ptype in PLOT_TYPES:
            try:
                replacement_plot = render_plot(
                    df, ptype, x=replacement.get("x"), y=replacement.get("y"),
                    hue=replacement.get("hue"), title=replacement.get("title"),
                )
            except Exception:
                replacement_plot = None
    return {
        ...
        "impact_notes": parsed.get("impact_notes", ""),
        "replacement_plot": replacement_plot,
    }
...
async def datalab_list_datasets(mid: str, user: Dict[str, Any] = Depends(current_user)):
    doc = await db.manuscripts.find_one(
        {"manuscript_id": mid, "user_id": user["user_id"]}, {"_id": 0, "datasets": 1},
    )
    if not doc:
        raise HTTPException(status_code=404, detail="Manuscript not found")
...
async def reference_search(q: str = Query(..., min_length=2), rows: int = 10, user: Dict[str, Any] = Depends(current_user)):
    ...
    params = {
        "query": q, "rows": min(max(rows, 1), 25),
        "select": "DOI,title,author,issued,container-title,volume,issue,page,publisher,type,abstract,URL",
        "sort": "relevance",
    }
    contact = os.environ.get("CROSSREF_CONTACT_EMAIL", "research@scholarpen.app")
    try:
        async with httpx.AsyncClient(timeout=15.0) as http:
            r = await http.get(url, params=params, headers={"User-Agent": f"ScholarPen/1.0 (mailto:{contact})"})
        r.raise_for_status()
    ...
    for it in items:
        ...
        out.append({
            ...
            "authors": authors, "year": year, "container": (it.get("container-title") or [""])[0],
            "volume": it.get("volume"), "issue": it.get("issue"), "page": it.get("page"),
            "publisher": it.get("publisher"), "type": it.get("type"), "url": it.get("URL"),
        })
    return {"results": out}
...
@api.get("/")
async def root():
    return {"app": "ScholarPen", "ok": True, "sections": SECTION_KEYS}
